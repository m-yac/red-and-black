  ...
  takeTurn() {
    for (let k = this.m+1; true; k++) {
      ...
      if ((bits | this.bitmask) == this.bitmask && this.board.players.length != 1 ||
        ...
        this.board.maxOccupiedRadius = Math.max(this.board.maxOccupiedRadius, cellRadius);
        ...
        this.m = k;
        this.lastOccupiedRadius = cellRadius;
        this.sequence.push([k, cellRadius]);
        // fullyScannedRadius is the largest r such that every player has
        // finished scanning radius r. A player at lastOccupiedRadius = L has
        // scanned every cell of radius < L plus some of radius L, so its
        // fully-scanned threshold is L - 1. Take the min across players.
        let minR = cellRadius;
        for (const p of this.board.players) {
          if (p.lastOccupiedRadius < minR) minR = p.lastOccupiedRadius;
        }
        this.board.fullyScannedRadius = minR - 1;
        return;
      }
      ...
      if (!this.board.isOccupied(i, j) && popcount32(this.board.bits.get(i, j)) > 1) {
        this.board.unoccupiedSequence.set(k, cellRadius);
      }
    }
  }
}
